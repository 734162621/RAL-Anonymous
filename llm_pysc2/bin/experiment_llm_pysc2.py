...
from llm_pysc2.cfg.llm_pysc2 import ConfigPysc2_Harass, ConfigPysc2_Defend, ConfigPysc2_Combat
from llm_pysc2.agents import MainAgent, LLMAgent, RAG_LLMAgent, RAG2_LLMAgent
import os
    ...
    for agent_name in list(config.AGENTS.keys()):
      for team in config.AGENTS[agent_name]['team']:
        team['task'] = [
          ...
          {'time': None, 'pos': None, 'info': "Kill as much as enemy Drones as possible until all units dead."},
        ]
        if team['name'] == 'AdeptPhase-1':
          team['task'][1] = {'time': None, 'pos': None, 'info': "assist the team Adept-1 to reposition or retreat."}
        if team['name'] == 'Adept-1':
          team['game_group'], team['select_type'] = 1, 'group'
        if team['name'] == 'Phoenix-1':
          team['game_group'], team['select_type'] = 1, 'group'
  elif task in [3]:
...
class MainAgentLLMPysc2(MainAgent):
  def __init__(self):
    ...
    super(MainAgentLLMPysc2, self).__init__(config, RAG2_LLMAgent)  # RAG_LLMAgent  RAG2_
...
if __name__ == "__main__":
  ...
  if not (enable_image_rgb or enable_image_feature):
    os.system(f"python -m pysc2.bin.agent --map {map_name} --agent_race protoss --parallel 2 "
