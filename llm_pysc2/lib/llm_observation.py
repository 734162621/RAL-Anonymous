...
from llm_pysc2.lib import game_event, llm_action
...
def get_teams_info(agent) -> str:
  ...
  for team in agent.teams:
    ...
    for i in range(len(team_obs_list)):
      ...
      team['b'] = edge_b = edge_b if edge_b == size_screen - 1 else edge_b - int(size_screen / 6)
...
def get_event_info(agent) -> str:
  event_text = ''
  event_dict = game_event.get_event(agent)
  ...
  for team in agent.teams:
    ...
    if len(event_dict[team['name']]) == 0:
      continue
    team_event_text = ''
    team_event = event_dict[team['name']][0]
...
class CombatGroupTranslatorO(BaseTranslatorO):
  ...
  def translate(self, agent) -> str:
    ...
    self.text_obs = self.state['game_info'] + self.state['units_info'] + self.state['event_info'] + self.state['knowledge_info'] + \
                    self.state['valid_actions'] + self.state['valid_args_explanation'] + self.state['last_action_info'] + \
                    self.state['last_action_error_info']   #
