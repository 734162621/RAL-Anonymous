...
PROTOSS_BASIC_ACTION_SMAC2 = [
  {'name': 'Attack_Unit', 'arg': ['tag'],
   'func': [(12, F.Attack_screen, ('queued', 'screen_tag')),
            ...
            # (2, F.select_point, ('select', 'screen_tag')),
            # (2, F.select_point, ('select', 'screen_tag'))
            ]},  # 使用select_point空选实现动作悬空，确保有后摇的攻击成功释放
...
def is_valid_screen_range(obs, screen, size_screen) -> (int, int, int, int):
  arr = obs.observation.feature_screen.pathable
  arr_t = arr.T
  ...
  edge_b = edge_b if edge_b == size_screen - 1 else edge_b - int(size_screen / 8)
    ...
    for line in lines:
      ...
      if start_recognize:
        if ("Team" in line and ":" in line) or ("team" in line and ":" in line):
          ...
          print(f"team_name={team_name}, team_action_name_list={team_action_name_list}")

          # if "*" in team_name:
          #   team_name_ = ''
          #   for charactor in team_name:
          #     team_name_ = team_name_ + charactor if charactor != "*" else team_name_
          ...
          if action_name not in team_action_name_list:
            continue
          if not first_function_attack and "Attack" in action_name and \
          ...
          if "[" in action_args:
            ratio = self.size_screen / SCREEN_WORLD_GRID
