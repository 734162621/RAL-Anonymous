...
def dify_retrieve(api_base, api_key, dataset_id, **kwargs):
  ...
  query = str(kwargs.get('query', 'None'))
  top_k = int(kwargs.get('top_k', 1))
  # if state == 'None':
  #   raise AssertionError(f"state needed")
  if query == 'None':
...
if __name__ == "__main__":
  ...
  api_key = 'dataset-cS2LCXGmve8rawYw6QWIJClh'
  dataset_id = 'e47a3827-f848-43ce-acc2-7af41532769b'
  document_id = '00a4fee6-bfef-4105-9553-c9ec3b879548'
  ...
  # # TODO: View Batch Indexing-Status
  # print('--' * 50 + f"\nCall dify_dataset_document_list()")
...
"""
    ...
    # # test: retrieval
    # kwargs = {'query': text_sars['s'], 'top_k': 3}
    # response = dify_retrieve(api_base, api_key, dataset_id, **kwargs)
    # pprint(response.json(), width=200)