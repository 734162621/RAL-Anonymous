...
from llm_pysc2.lib import llm_client, llm_observation, llm_action, llm_prompt, llm_communicate, task, utils
...
class LLMAgent:
  def __init__(self,
    ...
    for team in self.teams:
      ...
      team['obs_history'] = deque(maxlen=30)  # 30 agent step
      team['obs_last'] = []
  ...
  def _after_query(self, raw_text_a):
    ...
    self.action_errors = {}
    for team in self.teams:
      team['obs_history'].append(team['obs'])
  ...
  def query(self, obs) -> None:
    ...
    raw_text_a = self.get_text_a(text_o, base64_image=base64_image)  # query the llm and get response
    if self.name not in self.config.AGENTS_ALWAYS_DISABLE and self.enable:
      utils.write_to_file(json.dumps({self.main_loop_step: text_o}), self.log_dir_path + f"/{self.name}/o.txt")

    self._after_query(raw_text_a)
  ...
  def get_func(self, obs):  # 该函数需要将当前text-pysc2动作对应的下一个pysc2函数取出，确认函数和参数是合法的，然后交给到主智能体
    ...
    if not self.action_valid_check_1 and func_id != 12:
      enable_no_op = False
    ...
    return func_id, func_call, enable_no_op
